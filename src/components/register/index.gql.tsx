...
const RegisterGQL: FC = () => {
    ...
    const handleClick = async () => {
        ...
        //入力されたデータを引数にセット
        await registerMutation({variables: {userData: userData}}).then(() => {
            routeHistory.push(Path.login);
        }).catch( async (e) => {
            if( registerError?.graphQLErrors[0].extensions?.code === "BAD_REQUEST" || e.message === "入力されたメールアドレスは既に使用されています。"){
                await setEmailDuplicated(true)
            }
        })
    }

    return ( registerLoading ? (<LinearProgress style={{width: "60%", marginTop: "20%", marginLeft: "20%"}}/>) : (
        <div>
